...
let insertVideo = (value) => {
  let _sql = `insert into videos set name=?,country=?,classify=?,release_time=?,image=?,video_path=?,star=?,duration=?,detail=?;`
  return query(_sql, value)
}
...
let updateVideoHasImg = (value) => {
  let _sql = `update videos set name=?,country=?,classify=?,release_time=?,image=?,video_path=?,star=?,duration=?,detail=? where id=?;`
  return query(_sql, value)
}
